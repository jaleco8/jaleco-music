...
type PrepStartBlocker = 'none' | 'no-cues' | 'invalid-audio'
type LivePanel = 'speed' | 'repeat' | 'more' | null
type PresetConfig = {
  studyMode: StudyMode
  blindMode: boolean
  playbackRate: number
...
type LiveHelpState = { visible: boolean; step: 0 | 1 | 2 }
...
const LIVE_HELP_DISMISSED_KEY = 'jaleco-music:live-help-dismissed:v1'
...
function StudyExperience({
  allowEditing = false,
  title = 'Escucha y lee la historia',
  subtitle = 'Estado 1 · Preparación',
  ...
  const [studyMode, setStudyMode] = useState<StudyMode>('with-translation')
  const [blindMode, setBlindMode] = useState(false)
  const [repeatTarget, setRepeatTarget] = useState<RepeatTarget>(10)
  ...
  const [livePanel, setLivePanel] = useState<LivePanel>(null)
  ...
  const [activePreset, setActivePreset] = useState<PresetKey | null>('quick-start')
  ...
  const [liveHelp, setLiveHelp] = useState<LiveHelpState>({ visible: false, step: 0 })
  const [uiToast, setUiToast] = useState<{ tone: UiToastTone; message: string } | null>(null)
  ...
  const prepStartLabel =
    activePreset === 'without-translation'
      ? 'Iniciar sin traducción'
      : activePreset === 'interactive'
        ? 'Iniciar práctica interactiva'
        : activePreset === 'blind'
          ? 'Iniciar inmersión auditiva'
  ...
  const prepStartBlocker: PrepStartBlocker = audioError
    ? 'invalid-audio'
    : cues.length === 0
      ? 'no-cues'
      : 'none'
  const prepSummaryCopy =
    prepStartBlocker === 'invalid-audio'
      ? 'Audio con error. Revisa el archivo antes de empezar.'
      : prepStartBlocker === 'no-cues'
  ...
  const visiblePreviewCues = previewExpanded ? cues : cues.slice(0, 4)
  const currentLoop =
    repeatTarget === 'inf'
      ? completedLoops + 1
      : Math.min(completedLoops + 1, repeatTarget)
  ...
  useEffect(() => {
    if (initialViewMode === 'live' && cues.length === 0) {
      setViewMode('prep')
      return
    }

    setViewMode(initialViewMode)
  }, [cues.length, initialViewMode])
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        if (
          persisted.repeatTarget === 'inf' ||
          persisted.repeatTarget === 5 ||
          persisted.repeatTarget === 10 ||
          persisted.repeatTarget === 20 ||
          persisted.repeatTarget === 30 ||
          persisted.repeatTarget === 40
        ) {
          setRepeatTarget(persisted.repeatTarget)
        }
        if (
          persisted.playbackRate === 0.5 ||
          persisted.playbackRate === 0.75 ||
          persisted.playbackRate === 1 ||
          persisted.playbackRate === 1.25 ||
          persisted.playbackRate === 1.5
        ) {
          setPlaybackRate(persisted.playbackRate)
        }
        if (typeof persisted.focusMode === 'boolean') {
          setFocusMode(persisted.focusMode)
        }
        if (typeof persisted.countdownEnabled === 'boolean') {
          setCountdownEnabled(persisted.countdownEnabled)
        }
        if (typeof persisted.hapticEnabled === 'boolean') {
          setHapticEnabled(persisted.hapticEnabled)
        }
        if (
          persisted.lineSpacing === 'compact' ||
          persisted.lineSpacing === 'normal' ||
          persisted.lineSpacing === 'relaxed'
        ) {
          setLineSpacing(persisted.lineSpacing)
        }
        if (typeof persisted.fontLevel === 'number') {
          setFontLevel(clamp(Math.round(persisted.fontLevel), 0, FONT_LEVELS.length - 1))
        }
      ...
      const liveHelpDismissed = readStorage(window.localStorage, LIVE_HELP_DISMISSED_KEY)
      if (!liveHelpDismissed && initialViewMode === 'live') {
        setLiveHelp({ visible: true, step: 0 })
      }
    } catch {
      // Ignore malformed local state and keep defaults.
    } finally {
      setPrepStateHydrated(true)
      setUiToast({ tone: 'info', message: 'Sesión restaurada' })
    }
  }, [initialViewMode])
  ...
  const canEditTimings = allowEditing && (Boolean(audioUrl) || isAdvancedOpen)
  ...
  const pushUiToast = useCallback((message: string, tone: UiToastTone = 'info') => {
    setUiToast({ tone, message })
    ...
    }, tone === 'warning' || tone === 'error' ? 4000 : 2500)
  }, [])

  useEffect(() => {
    if (!uiToast || uiToastTimerRef.current !== null) {
      return
    }

    const timeout = uiToast.tone === 'warning' || uiToast.tone === 'error' ? 4000 : 2500
  ...
  const handlePrepStart = useCallback(() => {
    ...
    if (prepStartBlocker !== 'none') {
      trackEvent('prep_invalid_start', {
        blocker: prepStartBlocker,
        frases: cues.length,
        audio_cargado: Boolean(audioUrl),
      })
      pushUiToast(
        prepStartBlocker === 'invalid-audio'
          ? 'No pudimos leer este archivo de audio'
          : 'Agrega contenido para comenzar',
        prepStartBlocker === 'invalid-audio' ? 'error' : 'warning',
      )
      return
    }
  ...
  const returnToFollow = useCallback(() => {
    ...
    trackEvent('live_return_follow', {
      focus_cue: focusCueIndex,
    })
    pushUiToast('Siguiendo de nuevo', 'success')
  }, [focusCueIndex, pushUiToast, snapToFocusCue, trackEvent])
  ...
  const applyPreset = useCallback(
    (preset: PresetKey) => {
      ...
      pushUiToast('Preset aplicado', 'success')
    },
    [applyRepeatTarget, applyStudyMode, pushUiToast, trackEvent],
  )
  ...
  const handleAudioUpload = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith('audio/')) {
      setAudioError('No pudimos leer este archivo de audio')
      pushUiToast('No pudimos leer este archivo de audio', 'error')
  ...
  const toggleFullscreen = useCallback(async () => {
    try {
      ...
      pushUiToast('No pudimos activar pantalla completa', 'error')
    }
  }, [pushUiToast])
  ...
  useEffect(() => {
    ...
    const onLoadedMetadata = () => {
      if (Number.isFinite(audio.duration)) {
        ...
        setAudioError('')
        pushUiToast('Audio listo', 'success')
      }
    }
    ...
    const onError = () => {
      ...
      pushUiToast('No pudimos leer este archivo de audio', 'error')
      trackEvent('audio_upload_error', {
        file_name: audioLabel,
        file_type: 'audio-element-error',
      })
    }
  ...
  }, [audioLabel, finishPass, isActivePlayback, pushUiToast, trackEvent])
  ...
  useEffect(() => {
    if (viewMode !== 'live' || typeof window === 'undefined') {
      return
    }

    const dismissed = readStorage(window.localStorage, LIVE_HELP_DISMISSED_KEY)
    if (dismissed) {
      return
    }

    setLiveHelp((current) => (current.visible ? current : { visible: true, step: 0 }))
    trackEvent('live_help_seen')
  }, [trackEvent, viewMode])
  ...
  const handleStageScroll = () => {
    ...
    if (playbackStatus === 'playing') {
      ...
      setControlsVisible(true)
      trackEvent('live_enter_exploring', {
        cue_index: activeCueIndex,
      })
    }
  }
    ...
    <main className={liveClassName} style={{ '--line-height': LINE_HEIGHTS[lineSpacing] } as CSSProperties}>
      ...
      {viewMode === 'prep' && (
        <section className="prep-screen">
          ...
          {uiToast && <div className={`app-toast study-toast is-${uiToast.tone}`}>{uiToast.message}</div>}

          {!prepStateHydrated ? (
            <div className="prep-skeleton-grid">
              <div className="skeleton-block h-lg" />
              <div className="skeleton-block h-xl" />
              <div className="skeleton-block h-lg" />
            </div>
          ) : (
          <div className="prep-grid prep-grid-guided">
            <div className="field prep-editor prep-content-panel">
              <div className="field-header">
                <span>{allowEditing ? 'Contenido de la práctica' : '3. Contenido de la práctica'}</span>
                {allowEditing && (
              ...
              {!allowEditing && (
                <p className="field-hint">Vista previa del contenido listo para practicar. Solo administración puede editar texto y tiempos.</p>
              )}
              ...
              {!canEditTimings && (
                <p className="field-hint">
                  {allowEditing
                    ...
                    : 'Sincronización de tiempos disponible en el editor de administración.'}
                </p>
              )}
              ...
              {prepEditorTab === 'text' && (
                <>
                  {allowEditing ? (
                    ...
                    <div className="prep-preview-list">
                      {visiblePreviewCues.map((cue, index) => (
                        <article key={cue.id} className="prep-preview-row">
                          <span className="prep-preview-index">{index + 1}</span>
                          <div>
                            <p>{cue.text}</p>
                            {cue.translation && <span>{cue.translation}</span>}
                          </div>
                        </article>
                      ))}
                    </div>
                    ...
                    <div className="empty-state-card">
                      <h3>Esta sección todavía no tiene frases</h3>
                      <p>Agrega contenido para comenzar o vuelve al editor de administración.</p>
                    </div>
                  )}

                  {!allowEditing && cues.length > 4 && (
                    <button
                      type="button"
                      className="ghost-button small prep-preview-toggle"
                      onClick={() => setPreviewExpanded((current) => !current)}
                      data-interactive="true"
                    >
                      {previewExpanded ? 'Ver menos' : 'Ver todas'}
                    </button>
                  )}
            ...
            <div className="panel-card prep-goal-panel">
              <p className="panel-title">1. Elige tu objetivo</p>
              ...
              <p className="field-hint">2. Ajusta la sesión</p>
            ...
            <div className="panel-card panel-collapsible prep-audio-panel">
              ...
              <p className={`field-hint ${audioError ? 'is-error' : ''}`}>
                {audioError ? audioError : audioUrl ? `Audio listo: ${audioLabel}` : 'Sin audio cargado'}
              </p>
            ...
            <div className="panel-card panel-collapsible prep-advanced-panel">
              <div className="panel-headline">
                <p className="panel-title panel-title-with-icon">
          ...
          <footer className="prep-footer">
            <div className="prep-footer-copy">
              <p>{prepSummaryCopy}</p>
              {prepStartBlocker !== 'none' && (
                <span className="inline-feedback is-warning">
                  {prepStartBlocker === 'invalid-audio'
                    ? 'No pudimos leer este archivo de audio'
                    : 'Agrega contenido para comenzar'}
                </span>
            ...
            <button
              ...
              type="button"
              disabled={prepStartBlocker !== 'none'}
              data-interactive="true"
            >
      ...
      {viewMode === 'live' && (
        ...
        >
          ...
          {controlsVisible && (
            <header className="live-header" data-interactive="true">
              ...
              <div className="repeat-badge" data-interactive="true">
                <strong>
                  {repeatTarget === 'inf' ? `Vuelta ${currentLoop}` : `Vuelta ${currentLoop} de ${repeatTarget}`}
                </strong>
                <small>{remainingLoops === null ? 'Repetición continua' : `Restan ${remainingLoops}`}</small>
              </div>
          ...
          {playbackStatus === 'exploring' && (
            <div className="live-exploring-banner" aria-live="polite">
              Explorando manualmente. El texto dejó de seguirte.
            </div>
          )}
          ...
          {liveHelp.visible && (
            <div className="live-help-overlay" data-interactive="true" role="dialog" aria-modal="true">
              <div className="live-help-card">
                <p className="eyebrow">Cómo controlar esta práctica</p>
                <h2>Gestos rápidos</h2>
                <ol className="live-help-list">
                  <li className={liveHelp.step === 0 ? 'is-current' : ''}>
                    Toca una vez para ver u ocultar controles.
                  </li>
                  <li className={liveHelp.step === 1 ? 'is-current' : ''}>
                    Doble toque lateral para adelantar o retroceder 10 segundos.
                  </li>
                  <li className={liveHelp.step === 2 ? 'is-current' : ''}>
                    Arrastra horizontal para buscar y vertical para cambiar el tamaño del texto.
                  </li>
                </ol>
                <div className="live-help-actions">
                  <button
                    type="button"
                    className="ghost-button"
                    onClick={() => {
                      const nextStep = liveHelp.step < 2 ? ((liveHelp.step + 1) as 0 | 1 | 2) : 2
                      setLiveHelp((current) => ({ ...current, step: nextStep }))
                    }}
                    data-interactive="true"
                  >
                    Siguiente
                  ...
                  <button
                    ...
                    onClick={() => {
                      setLiveHelp({ visible: false, step: 0 })
                    }}
                    data-interactive="true"
                  >
                    Entendido
                  ...
                  <button
                    ...
                    className="ghost-button"
                    onClick={() => {
                      writeStorage(window.localStorage, LIVE_HELP_DISMISSED_KEY, 'true')
                      setLiveHelp({ visible: false, step: 0 })
                      trackEvent('live_help_dismissed')
                    }}
                    data-interactive="true"
                  >
                    No volver a mostrar
          ...
          {gestureMessage && (
            <div className="gesture-feedback" aria-live="polite">
              {gestureMessage}
            </div>
          )}

          {loopToast && (
            <div className="loop-toast" aria-live="polite">
              {loopToast}
            </div>
          )}

          {uiToast && viewMode === 'live' && (
            <div className={`app-toast study-toast live-toast is-${uiToast.tone}`} aria-live="polite">
              {uiToast.message}
            </div>
          )}
          ...
          {controlsVisible && (
            <>
              {livePanel && (
                <div className="control-tray" data-interactive="true">
                  {livePanel === 'speed' && (
                    <div className="chip-row compact" role="radiogroup" aria-label="Velocidad">
                      {SPEED_OPTIONS.map((option) => (
                        <ChipSelectable
                          key={option}
                          selected={playbackRate === option}
                  ...
                  {livePanel === 'repeat' && (
                    <div className="chip-row compact" role="radiogroup" aria-label="Repeticiones">
                      {REPEAT_OPTIONS.map((option) => (
                        <ChipSelectable
                          key={option}
                          selected={repeatTarget === option}
                  ...
                  {livePanel === 'more' && (
                    <div className="live-more-panel">
                      ...
                      <div className="live-more-actions">
                        <button className="ghost-button small" type="button" onClick={toggleFullscreen}>
                          {isFullscreen ? 'Salir' : 'Pantalla'}
                        </button>
                        <button
                          className="ghost-button small"
              ...
              <footer className="control-dock" data-interactive="true">
                ...
                <button
                  type="button"
                  className={`ghost-button small dock-pill ${livePanel === 'speed' ? 'is-open' : ''}`}
                  onClick={() => setLivePanel((current) => (current === 'speed' ? null : 'speed'))}
                  data-interactive="true"
                >
                  {playbackRate === 1 ? 'Velocidad 1x' : `Velocidad ${playbackRate}x`}
                </button>

                <button
                  type="button"
                  className={`ghost-button small dock-pill ${livePanel === 'repeat' ? 'is-open' : ''}`}
                  onClick={() => setLivePanel((current) => (current === 'repeat' ? null : 'repeat'))}
                  data-interactive="true"
                >
                  {repeatTarget === 'inf' ? 'Repeticiones ∞' : `Repeticiones x${repeatTarget}`}
                </button>

                <button
                  type="button"
                  className={`ghost-button small dock-pill ${livePanel === 'more' ? 'is-open' : ''}`}
                  onClick={() => setLivePanel((current) => (current === 'more' ? null : 'more'))}
                  data-interactive="true"
                >
                  Más
                </button>
